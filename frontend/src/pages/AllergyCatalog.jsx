...
import {
  createAllergyCatalogItem,
  deleteAllergyCatalogItem,
  getAllAllergyCatalog,
} from "../services/allergyCatalogService";
...
const AllergyCatalog = () => {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      const saved = await createAllergyCatalogItem({ name: String(newName || "").trim() });
      setItems((prev) => [...prev, saved]);
      setNewName("");
      toast.success("Allergie ajoutée au catalogue");
    } catch (err) {
      toast.error(getApiErrorMessage(err, "Impossible d'ajouter l'allergie."));
    } finally {
  ...
  const confirmDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      await deleteAllergyCatalogItem(itemIdToDelete);
  ...
  if (loading) {
    return (
      <DentistPageSkeleton
        title="Allergies"
        subtitle="Chargement du catalogue des allergies"
        variant="table"
      />
    );
  }
      ...
      {showConfirm && (
        <div className="modal-overlay">
          <div className="modal-content">
            <h2>Supprimer</h2>
            <p>Supprimer cette allergie du catalogue ?</p>
